...
class TemplateTextToSpeechStudioBuilder:
    ...
    def get_data_TextToSpeech_txt_model(self,data=[]):
       
           
            model_info = {
                "modelGateway": data.get("modelGateway", ""),
                "modelAi": data.get("modelAi", "wasmdashai/RunTasking"),
    ...
    def update_models(self, category,language,dialect):
        ...
        return gr.update(choices=["Male","Famle"], value="Male", visible=True),gr.update(choices=default_model, value=first_value,visible=True)
    def update_models_gender(self, category,language,dialect,gender):
        self.msg_event = f"Updating models for dialect {dialect}"
        default_model=self.get_filter(FilterModelAI(category=category,language=language,dialect=dialect,gender=gender),"name")
        self.msg_event = f"Updating models for dialect {dialect}"
...
class TemplateTextToSpeechBuilder:
    def __init__(self, url, token, isDev=True, data=None) -> None:
        self.msg_event = "Initialization started"
        self.status_code=444
        self.data=data
